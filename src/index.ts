...
import { formatResult, parseTest, runTest, runTestFiles } from "./runner.js";

// CLI mode: `terminal-driver-mcp run <test.json...>` replays test scripts and exits —
// no MCP server, no LLM. Everything below this block is server-only.
...
// stdout is reserved for MCP protocol traffic; route all logging to stderr,
// including any stray console.log from dependencies.
const log = (...args: unknown[]) => console.error("[terminal-driver-mcp]", ...args);
console.log = console.error;

const server = new McpServer({ name: "terminal-driver-mcp", version: "0.1.0" });
...
server.registerTool(
  ...
  safe(async ({ session_id, command, cwd, cols, rows }) => {
    const session = createSession(session_id, command, cols, rows, cwd);
...
server.registerTool(
  ...
  safe(async ({ command, cwd, timeout_ms, cols, rows }) => {
    ...
    const session = createSession(id, command, cols, rows, cwd, false);
    try {
      ...
      await waitForIdle(session, 50, 1000);
...
server.registerTool(
  ...
  safe(async ({ session_id, input, special_keys }) => {
    const session = getSession(session_id);
    if (session.exited) {
      return fail(`Session "${session_id}" has exited (code ${session.exitCode}); cannot write. Screen is still readable via session_read.`);
    }
...
server.registerTool(
  "session_wait_idle",
  {
    ...
    inputSchema: {
      session_id: sessionId,
      idle_ms: z.number().int().min(20).max(10000).default(80).describe("Quiet/unchanged period required to consider the terminal stable"),
...
server.registerTool(
  ...
  safe(async ({ file, test_json }) => {
    if (!file === !test_json) {
      return fail("Provide exactly one of 'file' (path to JSON test) or 'test_json' (inline JSON).");
    }
    const source = file ?? "inline test";
    const json = file ? readFileSync(file, "utf8") : test_json!;
...
server.registerTool(
  ...
  safe(async ({ session_id }) => ok(await killSession(session_id)))
);
